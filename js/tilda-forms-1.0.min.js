(function ($) {
    ...
    $(document).ready(function () {
        ...
        checkVerifyTildaCaptcha = function (event) {
            if (event.origin.indexOf('modul.club') != -1) {
        ...
        window.tildaForm.addTildaCaptcha = function ($jform, formskey) {
            ...
            $('body').append('<div id="tildaformcaptchabox" style="z-index:10000000;position:fixed; text-align: center; vertical-align: middle; top: 0px; left:0px; bottom: 0px; right: 0px; background: rgba(255,255,255,0.97);"><iframe id="captchaIframeBox" src="//modul.club/procces/captcha/?tildaspec-formid=' + $jform.attr('id') + '&tildaspec-formskey=' + formskey + '" frameborder="0" width="100%" height="100%"></iframe></div>');
            ...
            window.stripehandler.open({
                ...
                token: function (token, args) {
                    if (token && token.id) {
                        $.ajax({
                            type: "POST",
                            url: 'https://modul.club/payment/stripe/',
                            data: {
                ...
                if (arArgs && arArgs.html > '') {
                    ...
                    if ($jbankform.length > 0) {
                        ...
                        $jbankform.submit(function (e) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: 'https://modul.club/payment/banktransfer/',
        ...
        window.tildaForm.send = function ($jform, btnformsubmit, formtype, formskey) {
            ...
            if (formtype == 2 || (!formtype && formskey > '')) {
                ...
                $formurl = 'https://modul.club/procces.php';

                // дополнительно берем GET 
                var _dataParams = ($jform.serialize() || '') + '&' + (window.location.search.replace('?','') || '');

                $.ajax({
                    type: "POST", url: $formurl, data: _dataParams, dataType: "json",
                    success: function (json) {
                        // PAP
                        try {
                            PostAffTracker.setAccountId();
                            ...
                            var phone = ($jform.find('input[name=phone]').val() || '')
                                .replace('+7 ', '')
                                .replace('(', '')
                                .replace(')', '')
                                .replace(' ', '')
                            ...
                            var sale = PostAffTracker.createSale(); // инициализация заказа
                            sale.setOrderID(date + '_' + phone); // уник.ключ заказа
                            sale.setTotalCost(2500); // сумма по заказу
                            sale.setProductID('Прямая продажа'); // название товара для заказа
                            sale.setStatus('P'); // продажа
                ...
                if ($jform.data('is-formajax') == 'y') {
                    ...
                    if (attraction.indexOf('modul.club') != -1) {
